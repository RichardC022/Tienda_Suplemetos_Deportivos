...
@Component({
  selector: 'app-admin-ventas-manuales',
  standalone: true,
        ...
        <table class="vm-table">
          ...
          <tbody>
            @for (c of paginadas; track c.id) {
              ...
              <tr>
                <td colspan="8" class="vm-empty-row">No hay ventas manuales registradas</td>
              </tr>
            }
          </tbody>
        </table>
...
export class AdminVentasManualesComponent implements OnInit {
  todas: (Compra & { numeroFactura?: string })[] = [];
  filtradas: (Compra & { numeroFactura?: string })[] = [];
  paginadas: (Compra & { numeroFactura?: string })[] = [];
  terminoBusqueda = '';
  paginaActual = 1;
  porPagina = 10;
  totalPaginas = 1;
  ...
  cargar(): void {
    this.compraService.listarPorTipo('MANUAL').subscribe({
      next: (data) => {
        this.todas = data;
        this.filtradas = [...data];
        this.paginaActual = 1;
  ...
  onBuscar(): void {
    ...
    if (!t) {
      ...
      this.filtradas = this.todas.filter(c =>
        ...
        (c.persona?.documento || '').includes(t) ||
  ...
  generarPdf(): void {
    if (!this.filtradas.length) {
      this.toastService.show('No hay ventas manuales para generar informe', 'error');
      return;
    }
    this.pdfService.informeVentasManuales(this.filtradas);
    this.toastService.show('Informe PDF generado', 'exito');
  }

  eliminar(id: number): void {
    if (confirm('Seguro que desea eliminar esta venta manual?')) {
